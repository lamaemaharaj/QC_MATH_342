...
males <- list("Male Boomer" = maleBoomer, "Male GenX" = maleGenX, "Male Millenial" = maleMillennial)
females<- list("Female Boomer" = femaleBoomer, "Female GenX" = femaleGenX, "Female Millenial" = femaleMillennial)

complete <- list("M" = males, "F" = females)
complete
```
...
X = data.frame(
  ...
  name = rep(unlist(complete), times= 14*3), 
  y = rep(NA,n)
)
X
```
...
colnames(num_errors_by_parameter) = c("Threshold Parameters","Number Errors")
for (i in 1:n){
...
num_errors_by_parameter[order(num_errors_by_parameter[,"Number Errors"]),]
```
...
The threshold model's performance makes sense. The threshold we calculate of 5.4 looks like it fits between the two means. 
...
perceptron_learning_algorithm = function(Xinput, y_binary, MAX_ITER = 1000, w = NULL){
  ...
  for(iter in 1: MAX_ITER){
    for(i in 1:nrow(Xinput)){
      x_i = Xinput[i,]
      yh_i = ifelse(sum(x_i*w)>0,1,0)
      y_i = y_binary[i]
      for (j in 1:p){
        w[j] = w[j]+(y_i-yh_i)*x_i[j]
      }
    }
  }
  w
...
The first feature appears the x axies and the second feature appears. The independent vairable is idenfitied using the legend (key) on the side of plot. 
...
The bias in the y intercept and the second and third values are coeffiencets to x1 and x2. This is where x1 and x2 are weights of the first and second feautures of the model. 
...
I would have been more satisfied if the line that is plotted when between both of the distinct group of features instead of going through a point in our plot. I believe that the model is not wrong because it goes between two points. I think there was not enough data for the this model to be trained therefore it made such an error. 
...
Xy_simple_respon = as.numeric(Xy_simple$response)
...
The SVM is a better fit than the perceptron because it does not hit any of the data points. 
...
linear_svm_learning_algorithm = function(Xinput, y_binary, MAX_ITER = 5000, lambda = 0.1){
  n = nrow(Xinput)
  Xinput = as.matrix(cbind(1,n),Xinput)
  for(iter in 1: MAX_ITER){
    for(i in 1: nrow(Xinput)){
      x_i = Xinput[i,]
      y_i = y_binary[i]
      sum_hinge = max(0,0.5-(y_i-0.5)*(x_i*w-w))
      w = 1/n*(sum_hinge)+lambda*norm(w)^2
    }
  }
  w
}
...
```{r}
iris_manip = iris
iris_manip$Species = NULL
x_input = iris_manip
y_binary = iris$Species
xtest = c(3.0,5.7,0.25,2.3)
nn_algorithm_predict(x_input,y_binary,xtest,k=1)
```
...
nn_algorithm_predict = function(Xinput, y_binary, Xtest,k=1){
  ...
  for (j in 1:k){
    nearest_neighbors[j] = which.min[j]
  }
  counts = sort(nearest_neighbors,decreasing=TRUE)
